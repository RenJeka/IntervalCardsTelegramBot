...
export class LLMService {
  ...
  private static readonly defaultModel =
    process.env.OPENROUTER_MODEL || DEFAULT_LLM_MODEL;
      ...
      const response = await axios.post<LLMCompletionResponse>(
        ...
        requestBody,
        {
          headers: {
            ...
            "HTTP-Referer":
              process.env.APP_URL ||
              "https://github.com/RenJeka/IntervalCardsTelegramBot",
    ...
    if (axios.isAxiosError(error)) {
      ...
      if (axiosError.response) {
        ...
        switch (status) {
          case 401:
            throw new Error(
              "LLM service authentication failed. Please check API key.",
            );
          case 429:
            throw new Error(
          ...
          default:
            throw new Error(
              `LLM service error: ${errorData?.error?.message || "Unknown error"}`,
            );
        }
...
export class LLMHelper {
  ...
  static async generateWordSet(
    ...
    try {
      ...
      const userPrompt = `Generate ${count} basic vocabulary words for these categories: ${categoriesString} in ${learningLanguage} with translations to ${nativeLanguage}.`;
  ...
  private static getCacheKey(
    ...
    return `${categories.sort().join(",")}:${learningLanguage}:${nativeLanguage}:${count}`;
  }
