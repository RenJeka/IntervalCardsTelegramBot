...
- **Роль:** низькорівневий сервіс для взаємодії з OpenRouter API.
- **Функціональність:** надсилання запитів на завершення чату (chat completions), обробка помилок API (authentication, rate limits, timeouts), логування помилок.
- **Технології:** `axios` для HTTP запитів.

### 4.8 `LLMHelper`

- **Роль:** високорівневий хелпер для бізнес-логіки, пов'язаної з LLM.
- **Функціональність:** генерація наборів слів за категоріями та мовами, парсинг JSON відповідей від LLM, валідація отриманих даних.
- **Кешування:** реалізує in-memory кешування згенерованих наборів слів з TTL 24 години (через `LLM_CACHE_TTL_MS`).
