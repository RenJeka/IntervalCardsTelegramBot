...
export class DbAwsService implements IDbService {
    ...
    constructor() {
        ...
        if (!this.dynamoDbRegion || !this.tableNames.words) {
            throw new Error(
                'AWS_REGION or AWS_WORDS_TABLE_NAME are not defined'
            );
        }
    ...
    async writeWordByUserId(
        userId: number,
        message: string,
        checkDuplicates: boolean = true
    ): Promise<DbResponse> {
        try {
            const parsedRawItem: UserRawItemAWS =
                CommonHelper.parseUserRawItem(message);

            if (
                checkDuplicates &&
                (await this.checkDuplicate(userId, parsedRawItem.word))
            ) {
                return {
                    success: false,
            ...
            const response: PutItemCommandOutput = (await this.client.send(
                command
            )) as PutItemCommandOutput;
            if (response?.$metadata?.httpStatusCode !== 200) {
                throw new Error(
                    `❌️Something went wrong while writing word to DB: ${LogService.safeStringify(response)}`
                );
            }
            ...
            return {
                ...
                consumedCapacity: LogService.safeStringify(
            ...
            return {
                ...
                message: error.message
                    ? error.message
                    : 'Something wrong while writing word to DB',
            };
        }
    }
    ...
    async removeWordById(userId: number, wordId: string): Promise<DbResponse> {
        ...
        try {
            ...
            const deleteItemResponse: DeleteItemCommandOutput =
                (await this.client.send(
                    deleteItemCommand
                )) as DeleteItemCommandOutput;
            if (getItemResponse?.$metadata?.httpStatusCode !== 200) {
                throw new Error(
                    `❌️Something went wrong while deleting word to DB: ${LogService.safeStringify(deleteItemResponse)}`
                );
            }
            ...
            return {
                ...
                message: error.message
                    ? error.message
                    : '❌️Something wrong while deleting word from DB',
            };
        }
    }
    ...
    async setUserStatus(
        ...
        const response: UpdateItemCommandOutput = (await this.client.send(
            command
    ...
    async getUserStatus(userId?: number): Promise<UserStatus | null> {
        ...
        try {
            ...
            const response: ScanCommandOutput = (await this.client.send(
                command
            ...
            throw new Error(
                `Something wrong while scanning DynamoDB: ${LogService.safeStringify(error)}`
            );
        }
    }
    ...
    async setUserLanguage(
        ...
        if (response?.$metadata?.httpStatusCode !== 200) {
            throw new Error(
                `❌️Something went wrong while writing language to DB: ${LogService.safeStringify(response)}`
            );
        }
    ...
    async setLearningLanguage(
        ...
        if (response?.$metadata?.httpStatusCode !== 200) {
            throw new Error(
                `❌️Something went wrong while writing learning language to DB: ${LogService.safeStringify(response)}`
            );
        }
    ...
    async addUserFavoriteCategory(
        ...
        if (response?.$metadata?.httpStatusCode !== 200) {
            throw new Error(
                `❌️Something went wrong while writing favorite category to DB: ${LogService.safeStringify(response)}`
            );
        }
        return {
            success: true,
    ...
    async removeUserFavoriteCategory(
        userId: number,
        category: string
    ): Promise<DbResponse> {
        const updateItemParams: UpdateItemCommandInput = {
        ...
        if (response?.$metadata?.httpStatusCode !== 200) {
            throw new Error(
                `❌️Something went wrong while removing favorite category to DB: ${LogService.safeStringify(response)}`
            );
        }
        return {
            success: true,
    ...
    async getUserFavoriteCategories(userId: number): Promise<string[]> {
        ...
        try {
            ...
            return item.favoriteCategories
                ? Array.from(item.favoriteCategories)
                : [];
        } catch (error) {
            throw new Error(
                `Something wrong while scanning DynamoDB: ${LogService.safeStringify(error)}`
    ...
    async getUserDictionary(userId: number): Promise<UserItemAWS[]> {
        ...
        try {
            ...
            const items: UserItemAWS[] = response.Items?.map((item) =>
                unmarshall(item)
            ) as UserItemAWS[];

            items.sort((prev: UserItemAWS, next: UserItemAWS) =>
                prev.word.localeCompare(next.word)
            );
            return items;
        } catch (error) {
            throw new Error(
    ...
    async getFlatUserDictionary(userId: number): Promise<string[]> {
        ...
        return (await this.getUserDictionary(userId)).map(
            (word: UserItemAWS) => word.word
        );
    }
    ...
    async getAllUsersWithStatus(
        userStatus: UserStatus = UserStatus.START_LEARN
    ): Promise<UserDataAWS[]> {
        const scanInput: ScanCommandInput = {
            TableName: this.tableNames.users,
            ReturnConsumedCapacity: 'INDEXES',
        ...
        try {
            ...
            return response.Items.map(
                (item) => unmarshall(item) as UserDataAWS
            );
        } catch (error) {
    ...
    private async initDb(): Promise<void> {
        try {
            ...
            for (const tableKey in this.tableNames) {
                const tableName =
                    this.tableNames[tableKey as keyof typeof this.tableNames];
                if (!tables.includes(tableName)) {
                    throw new Error(`Table ${tableName} not found in DynamoDB`);
                }
            }
            LogService.info(
                chalk.green.bold(`✔ connecting to DynamoDB is successfully!`)
            );
        } catch (error) {
            throw new Error(
                `Failed to connect to DynamoDB or find required tables: ${error}`
            );
        }
    }
    ...
    private async listTables() {
        ...
        try {
            ...
            LogService.error(
                chalk.red`Error while list tables: ${LogService.safeStringify(error)}`
            );
        }
    }
